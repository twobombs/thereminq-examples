...
#    {'width': 96, 'depth': 14, 'trial': 1, 'square_magnetization': 0.00086, ...}
#    {'width': 96, 'depth': 15, 'trial': 1, 'square_magnetization': 0.00046, ...}
...
def create_heatmap_from_log(file_path='fullog.txt'):
    ...
    and generates a heatmap of square magnetization vs. qubit width and circuit depth. # MODIFIED
    ...
    try:
        ...
        # --- MODIFIED: Ensure 'square_magnetization' column is present ---
        required_cols = ['width', 'depth', 'square_magnetization']
        if not all(col in df.columns for col in required_cols):
            print(f"Error: The log file must contain the columns: {', '.join(required_cols)}")
            return

        # --- MODIFIED: Pivot using 'square_magnetization' as the value ---
        # Index: qubit width, Columns: circuit depth, Values: square_magnetization.
        # If there are multiple trials for the same width/depth, this will average them.
        heatmap_data = df.pivot_table(index='width', columns='depth', values='square_magnetization')
        ...
        # --- MODIFIED: Update titles and labels for clarity ---
        plt.title('Square Magnetization vs. Qubit Width and Circuit Depth', fontsize=16)
        plt.xlabel('Circuit Depth', fontsize=12)
        plt.ylabel('Qubit Width', fontsize=12)

        # --- MODIFIED: Update the saved filename ---
        ...
        output_filename = 'qubit_square_magnetization_heatmap.png'
