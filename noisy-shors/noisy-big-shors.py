...
try:
    from pyqrack import QrackSimulator # Changed import
    PYQRACK_AVAILABLE = True
except ImportError:
    PYQRACK_AVAILABLE = False
    QrackSimulator = None # Define as None if import fails, for robustness
...
def controlled_modular_multiplier_permutation(qsim, control_qubit, target_qubits, factor, N):
    ...
    THIS IS THE PRIMARY BOTTLENECK FOR LARGE N.
    """
    ...
    for s_in in range(N): # This loop runs N times!
        s_out = (s_in * factor) % N
...
def quantum_period_finding_subroutine(qsim, num_control_qubits, num_target_qubits, base_a, N):
    ...
    print(f"  WARNING: This step will be EXTREMELY slow for N={N} due to the naive oracle implementation.")
    controlled_modular_exponentiation(qsim, control_register, target_register, base_a, N)
    print(f"  Controlled modular exponentiation finished (if it ever does for N={N}).")
...
def shor_factor_pyqrack(N_to_factor, num_classical_trials=10, fixed_a=None):
    ...
    for trial_num in range(num_classical_trials):
        ...
        # Ensure QrackSimulator is available before trying to instantiate
        if not PYQRACK_AVAILABLE or QrackSimulator is None:
             print("  PyQrack's QrackSimulator not available. Skipping quantum step.")
             # Potentially fall back to classical or simply fail the trial
             continue


        qsim = QrackSimulator(total_qubits_needed) # Changed instantiation
...
if __name__ == "__main__":
    ...
    # Factors are 1009 and 1049. N.bit_length() = 21.
    # t_control_qubits = 2*21 = 42. m_target_qubits = 21. Total qubits = 63.
    N_to_factor = 1058441 


    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    print("!!! WARNING: THIS SIMULATION WILL BE EXTREMELY SLOW FOR THE CHOSEN N.    !!!")
    print(f"!!! Factoring N={N_to_factor} requires ~{N_to_factor.bit_length()*3} qubits and the quantum oracle   !!!") # Corrected string formatting
    print("!!! implementation is naive, leading to very long simulation times.        !!!")
    print("!!! This is for demonstrating qubit scaling, not for practical factoring.  !!!")
    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    ...
    if result_factors and result_factors[0] is not None:
        ...
        print(f"\nCould not factor {N_to_factor} with the given attempts using the quantum routine (or it was interrupted due to extreme runtime).")
