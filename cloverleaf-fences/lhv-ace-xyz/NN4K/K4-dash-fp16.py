...
#   ket_cache + resurrect_sim (within-worker sim recovery on context loss).
#   try/except RuntimeError in kicks/trotter/tomo (context-loss catching).
#   NaN guards in tomo, zz_exact guard, mpc.wait gather.
#   All Rev 305-310 NaN/variance/kick guards.
...
GATHER_TIMEOUT_S   = 120.0  # covers 28s JIT + 5s sleep + alloc + tomo
NAN_DRIVER_SLEEP_S = 5.0   # fixed GPU ring reset wait; replaces exponential backoff
...
def generate_manifold_topology() -> Tuple[List[Tuple[int, int]], Dict[int, List[int]]]:
    edges = set()
    ...
    if len(intra_edges) != _EXPECTED_RIM_EDGES:
        raise RuntimeError(f"Fatal: expected {_EXPECTED_RIM_EDGES} rim edges, got {len(intra_edges)}")
    seen_q = set()
    for a, b in intra_edges:
        seen_q.add(a); seen_q.add(b)
    if len(seen_q) != QUBITS_PER_MANIFOLD:
        raise RuntimeError(f"Fatal: rim edges cover {len(seen_q)} qubits, expected {QUBITS_PER_MANIFOLD}")
    ...
    return intra_edges, {k: edge_local_qubits(k) for k in range(LOCAL_EDGES)}
...
def build_junction_table(twist: bool) -> List[Dict[str, Any]]:
    ...
    for ji, (ma, mb) in enumerate(JUNCTION_PAIRS):
        ka = junction_slot(ma, mb); kb = junction_slot(mb, ma)
        qa_list = edge_local_qubits(ka); qb_list = edge_local_qubits(kb)
        bonds = [(qa_list[i], qb_list[pairing[i]]) for i in range(QUBITS_PER_EDGE)]
        table.append({"index": ji, "ma": ma, "mb": mb,
                      "slot_a": ka, "slot_b": kb,
                      "qubits_a": qa_list, "qubits_b": qb_list,
                      "bonds": bonds, "label": f"J{ma}{mb}"})
    ...
    for m, slots in used.items():
        if sorted(slots) != list(range(LOCAL_EDGES)):
            raise RuntimeError(f"Fatal: manifold {m} slot usage {sorted(slots)}")
    return table

def compute_kicks_and_junction_energy(
        profiles, noise, junctions, g_junction
        ) -> Tuple[Dict, float, Dict]:
    kicks: Dict[int, Dict[int, Tuple[float, float, float]]] = {m: {} for m in range(N_MANIFOLDS)}
    def site_vec(m, q):
        prof = profiles[m]; nx, ny, nz = noise[m].get(q, (0.0, 0.0, 0.0))
        return (float(prof["X"][q])+nx, float(prof["Y"][q])+ny, float(prof["Z"][q])+nz)
    def add_kick(m, q, g, vec):
        k = kicks[m].get(q, (0.0, 0.0, 0.0))
        kicks[m][q] = (k[0]+g*vec[0], k[1]+g*vec[1], k[2]+g*vec[2])
    energy = 0.0; junction_vectors: Dict[int, np.ndarray] = {}
    for rec in junctions:
        ma, mb = rec["ma"], rec["mb"]; acc = np.zeros(3); n_sites = 0
        for (qa, qb) in rec["bonds"]:
            va = site_vec(ma, qa); vb = site_vec(mb, qb)
            energy += -g_junction * (va[0]*vb[0] + va[1]*vb[1] + va[2]*vb[2])
            add_kick(ma, qa, g_junction, vb); add_kick(mb, qb, g_junction, va)
            acc += np.array(va) + np.array(vb); n_sites += 2
        junction_vectors[rec["index"]] = acc / max(1, n_sites)
    return kicks, energy, junction_vectors

def loop_momentum_amplitudes(junction_vectors, loop_indices):
    n = len(loop_indices)
...
def swap_asymmetry(profiles, junctions, pairs):
    out = {}; by_pair = {(r["ma"], r["mb"]): r for r in junctions}
    for (ma, mb) in pairs:
        rec = by_pair.get((min(ma, mb), max(ma, mb)))
        if rec is None: continue
        diffs = []
        for (qa, qb) in rec["bonds"]:
            if rec["ma"] == ma: pa, pb, ia, ib = profiles[ma], profiles[mb], qa, qb
            else: pa, pb, ia, ib = profiles[mb], profiles[ma], qa, qb
            for comp in ("X", "Y", "Z"):
                diffs.append(float(pa[comp][ia]) - float(pb[comp][ib]))
        out[f"M{ma}M{mb}"] = float(np.sqrt(np.mean(np.array(diffs)**2)))
    return out
...
def gpu_worker_process(
    ...
    try:
        ...
        if PZ is None: raise RuntimeError("Fatal: could not autodetect PZ")
        del _probe_z
        ...
        if PX is None: raise RuntimeError("Fatal: could not autodetect PX")
        del _probe_x
        ...
        if PY is None: raise RuntimeError("Fatal: could not autodetect PY")
        del _probe_y
        ...
        if abs(_corrected + 1.0) < 0.1:
            ...
            raise RuntimeError(f"Fatal: ANGLE_SCALE ambiguous: {_corrected:.6f}")
        del _sim_mag

        def is_context_loss(exc):
            msg = str(exc).lower()
        ...
        def apply_kicks(sim, kicks, dt_local):
            ...
            for raw_q, (kx, ky, kz) in kicks.items():
                ...
                _ = sim.pauli_expectation([0], [PZ])   # per-kick drain

        intra_edges, _edge_groups = generate_manifold_topology()
        ket_cache: Dict[int, np.ndarray] = {}

        def resurrect_sim(m: int):
            if m in sims:
                try: _old = sims.pop(m); del _old
                except Exception: pass
            gc.collect()
            sim_new = QrackSimulator(qubit_count=QUBITS_PER_MANIFOLD,
                                     is_binary_decision_tree=False,
                                     is_stabilizer_hybrid=False, is_gpu=True)
            if m in ket_cache:
                try:
                    sim_new.in_ket(ket_cache[m].tolist())
                    print(f"[Worker {rank}] M{m} resurrected from ket.", file=sys.stderr)
                except Exception as e:
                    print(f"[Worker {rank}] M{m} ket restore failed ({e}); |+>^15.", file=sys.stderr)
                    for q in range(QUBITS_PER_MANIFOLD): apply_h(sim_new, q)
            else:
                for q in range(QUBITS_PER_MANIFOLD): apply_h(sim_new, q)
                print(f"[Worker {rank}] M{m} resurrected to |+>^15 (no ket).", file=sys.stderr)
            sims[m] = sim_new
            return sim_new
        ...
        for m in assigned_manifolds:
            ...
            sims[m] = sim
            try: _ = sim.pauli_expectation([0], [PZ])
            except Exception as e:
                raise RuntimeError(f"Fatal: VRAM alloc failed M{m} rank {rank}: {e}")
            try: ket_cache[m] = np.array(sim.out_ket(), dtype=np.complex64)
            except Exception: pass
            ...
            for m in assigned_manifolds:
                ...
                if kick_payloads[m]:
                    try:
                        ...
                        if is_context_loss(_ke):
                            print(f"[Worker {rank}] t={t} M{m} context loss in kicks; resurrecting.", file=sys.stderr)
                            sim = resurrect_sim(m)
                            try: apply_kicks(sim, kick_payloads[m], dt)
                            except Exception: pass
                        else: raise

                # 2. Checkpoint after kicks
                try: ket_cache[m] = np.array(sim.out_ket(), dtype=np.complex64)
                except Exception: pass
                ...
                try:
                    ...
                    if is_context_loss(_te):
                        print(f"[Worker {rank}] t={t} M{m} context loss in trotter; resurrecting.", file=sys.stderr)
                        sim = resurrect_sim(m)
                        try:
                            trotter_step_body(sim, QUBITS_PER_MANIFOLD, intra_edges,
                                              current_J, current_hx, current_hz, dt)
                ...
                if is_measure:
                    ...
                    try:
                        ...
                        if is_context_loss(_me):
                            print(f"[Worker {rank}] t={t} M{m} context loss in tomo; resurrecting.", file=sys.stderr)
                            sim = resurrect_sim(m)
                            Z_arr = z_means(sim, all_q)
                            X_arr = x_means(sim, all_q)
                            Y_arr = y_means(sim, all_q)
                    ...
                    # 5. NaN guard (secondary: silent NaN from fp16)
                    if not (np.all(np.isfinite(Z_arr))
                        ...
                        print(f"[Worker {rank}] NaN tomo M{m} t={t}; "
                              f"sleeping {NAN_DRIVER_SLEEP_S:.0f}s + reinit.", file=sys.stderr)
                        try: del sims[m]
                        ...
                        if not math.isfinite(_sanity) or abs(_sanity - SIGN_X) > 0.5:
                            print(f"[Worker {rank}] GPU broken after reinit M{m}; 5s + dummy.", file=sys.stderr)
                            time.sleep(5.0)
                            try: del sims[m]
                            except KeyError: pass
                            gc.collect()
                    ...
                    try:
                        ...
                        if not _warned_fidelity:
                            print(f"[Worker {rank}] Warning: get_unitary_fidelity() not available.", file=sys.stderr)
        ...
        for m in list(sims.keys()):
            _s = sims.pop(m); del _s
        sims.clear(); gc.collect(); conn.close()
...
class K4ManifoldEngine:
    def __init__(self, master_seed: int = 1337, twist: bool = TWIST_GLUING):
        ...
        self.loop_indices     = [self.junction_by_pair[jp]["index"] for jp in LOOP_JUNCTIONS]
        self.chord_indices    = [self.junction_by_pair[jp]["index"] for jp in CHORD_JUNCTIONS]
    ...
    def _get_last_good(self, m):
        if self.lattice_history:
            ...
            return {"X": last[m, :, 0].copy(), "Y": last[m, :, 1].copy(), "Z": last[m, :, 2].copy()}
        return {"X": np.zeros(QUBITS_PER_MANIFOLD), "Y": np.zeros(QUBITS_PER_MANIFOLD), "Z": np.ones(QUBITS_PER_MANIFOLD)}
        ...
        try:
            with open(self.config_file, 'w') as f:
                json.dump({"topology": "K4_trihole_manifold_cluster", "revision": 316,
                           ...
                           "qubits_per_junction": QUBITS_PER_EDGE, "n_junctions": N_JUNCTIONS,
                           "unique_qubits": UNIQUE_QUBITS, "intra_edges": len(self.intra_edges),
                           ...
                           "gpus": GPUS_AVAILABLE, "manifold_vram_mb": MANIFOLD_VRAM_MB,
                           "fp16_lib": FP16_LIB}, f)
            with open(self.energy_csv, mode='w', newline='') as f:
                csv.DictWriter(f, fieldnames=["Step", "Anneal_Percent", "Exact_Bulk_Energy",
                    "Junction_Energy", "Total_Energy", "Min_Unitary_Fidelity",
                    "Swap_Asym_M0M1", "Swap_Asym_M2M3"]).writeheader()
            with open(self.profiles_csv, mode='w', newline='') as f:
                csv.DictWriter(f, fieldnames=["Step", "Junction", "Kind",
                    "X_mean", "Y_mean", "Z_mean"]).writeheader()
            with open(self.loop_csv, mode='w', newline='') as f:
                csv.DictWriter(f, fieldnames=["Step", "Loop_Momentum",
                    "Amp_X", "Amp_Y", "Amp_Z"]).writeheader()
        except Exception as e:
            print(f"[CSV] Warning: {e}", file=sys.stderr)
    ...
    def _log_csvs(self, step, anneal, bulk, junction_e, total, min_fidelity,
        ...
        try:
            with open(self.energy_csv, mode='a', newline='') as f:
                csv.DictWriter(f, fieldnames=["Step", "Anneal_Percent", "Exact_Bulk_Energy",
                    "Junction_Energy", "Total_Energy", "Min_Unitary_Fidelity",
                    "Swap_Asym_M0M1", "Swap_Asym_M2M3"]).writerow({
                    "Step": step, "Anneal_Percent": anneal, "Exact_Bulk_Energy": bulk,
                    "Junction_Energy": junction_e, "Total_Energy": total,
            ...
            with open(self.profiles_csv, mode='a', newline='') as f:
                w = csv.DictWriter(f, fieldnames=["Step", "Junction", "Kind", "X_mean", "Y_mean", "Z_mean"])
                for rec in self.junctions:
                    v = junction_vectors[rec["index"]]
                    kind = "loop" if rec["index"] in self.loop_indices else "chord"
                    w.writerow({"Step": step, "Junction": rec["label"], "Kind": kind,
                                "X_mean": float(v[0]), "Y_mean": float(v[1]), "Z_mean": float(v[2])})
            with open(self.loop_csv, mode='a', newline='') as f:
                w = csv.DictWriter(f, fieldnames=["Step", "Loop_Momentum", "Amp_X", "Amp_Y", "Amp_Z"])
    ...
    def run(self, total_steps: int, dt: float, initial_hx: float,
        ...
        if total_steps < 1: raise ValueError("total_steps must be at least 1")
        if measure_every < 1: raise ValueError("measure_every must be a positive integer")
        if not (0 <= probe_junction < N_JUNCTIONS): raise ValueError("probe_junction out of range")
        ...
        for rank in active_ranks:
            ...
            workers.append(p); pipes.append(parent_conn)
        ...
        try:
            for t in range(total_steps):
                ...
                # --- GATHER with mpc.wait + respawn on timeout ---
                manifold_states = {}
                bulk_energy     = 0.0
                max_lat_trotter = 0.0
                max_lat_tomo    = 0.0
                        ...
                        try:
                            ...
                            for m, payload in data.items():
                                ...
                                if any(np.any(~np.isfinite(state[c])) for c in ("X", "Y", "Z")):
                                    print(f"[Master] NaN M{m} step {t}; using last-good.", file=sys.stderr)
                                    state = self._get_last_good(m)
                                manifold_states[m] = state
                                bulk_energy += (payload["exact_bulk_energy"]
                                                if math.isfinite(payload["exact_bulk_energy"]) else 0.0)
                            ...
                            print(f"[Master] EOF rank {w_rank}; using last-good.", file=sys.stderr)
                            for m in self.worker_assignments[w_rank]:
                                manifold_states[m] = self._get_last_good(m)
                ...
                if len(self.lattice_history) % 10 == 0:
                    try: np.save(self.state_dump_file, np.array(self.lattice_history))
                    except Exception as e: print(f"[Checkpoint] {e}", file=sys.stderr)

                # --- NOISE ---
                scale = np.sqrt(dt / effective_shots)
                noise = {m: {} for m in range(N_MANIFOLDS)}
                for rec in self.junctions:
                    rng_j = np.random.default_rng([self.master_seed, t, rec["index"]])
                ...
                kicks, junction_energy, junction_vectors = \
                    compute_kicks_and_junction_energy(manifold_states, noise,
                                                      self.junctions, current_g_junction)
                ...
                if probe_step >= 0 and t == probe_step and probe_amplitude != 0.0:
                    ...
                    print(f"[Probe] step {t}: +{probe_amplitude:.4f} at {rec['label']}")

                # --- DIAGNOSTICS ---
                loop_amps = loop_momentum_amplitudes(junction_vectors, self.loop_indices)
                asym      = swap_asymmetry(manifold_states, self.junctions, MOEBIUS_PAIRS)
                ...
                for i, w_rank in enumerate(active_ranks):
                    pipes[i].send({m: kicks[m] for m in self.worker_assignments[w_rank]})
            ...
            if self.lattice_history:
                try:
                    ...
                    print(f"\n[Master] Failed to save: {e}", file=sys.stderr)
            for p in workers:
                p.join(timeout=15)
                if p.is_alive():
...
if __name__ == "__main__":
    ...
    try:
        engine.run(
            total_steps=100, dt=0.04, initial_hx=3.0,
            target_g_junction=0.15, target_J=1.0,
            target_hx=0.5, target_hz=0.2,
            measure_every=1, effective_shots=512.0,
            probe_step=-1, probe_amplitude=0.5, probe_junction=0
        )
