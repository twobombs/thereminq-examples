...
#
# REVISION 69 - MULTI-GPU DISTRIBUTED IPC ARCHITECTURE (PROD GOLD MASTER)
...
def gpu_worker_process(
    ...
    try:
        # --- HARDENED SIGN & MAGNITUDE CONVENTION AUTODETECT ---
        _sim_x = QrackSimulator(qubit_count=1, is_qubdd=True)
        _sim_x.h(0)
        ...
        _sim_z = QrackSimulator(qubit_count=1, is_qubdd=True)
        ...
        # Definitively verify r() implements half-angle exp(-i * theta/2 * P)
        _sim_mag = QrackSimulator(qubit_count=1, is_qubdd=True)
        _sim_mag.r(PX, np.pi, 0)
        mag_check = _sim_mag.pauli_expectation([0], [PZ])
        assert abs(SIGN_Z * mag_check + 1.0) < 1e-3, f"Fatal: r(PX,pi) magnitude check failed. Expected ~ -1.0, got SIGN_Z*mag_check={SIGN_Z * mag_check}"
        del _sim_mag
        # -------------------------------------------------------
        ...
        for p in assigned_patches:
            sim = QrackSimulator(qubit_count=QUBITS_PER_PATCH, is_qubdd=True)
        ...
        for t in range(total_steps):
            ...
            for p in assigned_patches:
                ...
                trotter_step_body(sim, QUBITS_PER_PATCH, intra_edges, current_J, current_hx, current_hz, dt)
...
class MultiGpuHadronEngine:
    ...
    def run(self, total_steps: int, dt: float, target_g_face: float, target_J: float,
            target_hx: float, target_hz: float, measure_every: int = 1, effective_shots: float = 512.0):
        
        assert total_steps >= 1, "total_steps must be at least 1"
        assert measure_every >= 1, "measure_every must be a positive integer"
        
        print(f"[Engine] {TOTAL_PATCHES} patches, {WORKER_GPUS} GPUs, {total_steps} steps")
        ...
        for rank in active_ranks:
            ...
            p.start()
            child_conn.close()  # Drop parent's copy of the write end to guarantee EOF propagation
            workers.append(p)
        ...
        try:
            for t in range(total_steps):
                ...
                if len(patch_full_states) != TOTAL_PATCHES:
                    raise RuntimeError(f"Fatal: IPC gather incomplete. Expected {TOTAL_PATCHES} patches, got {len(patch_full_states)}.")
                ...
                for p, prof in patch_profiles.items():
                    ...
                    stochastic_noise[p] = {q: (xn[i], yn[i], zn[i]) for i, q in enumerate(self.all_boundary_qubits)}
                ...
                for i, w_rank in enumerate(active_ranks):
                    worker_payload = {p: next_kick_payloads[p] for p in self.worker_assignments[w_rank]}
            ...
            for p in workers:
                # 15s timeout to allow heavy OpenCL VRAM free on QBDD destructors
                p.join(timeout=15)
                if p.is_alive():
                    p.terminate()
                    p.join(timeout=3)
