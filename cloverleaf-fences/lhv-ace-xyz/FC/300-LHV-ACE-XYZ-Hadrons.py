...
class HierarchicalHadronEngine:
    def __init__(self, device_ids: List[int], intra_topology: str = "RING", boundary_size: int = 4):
        ...
        # FIX: Guardrail for topology constants
        ...
        for i, pipe in enumerate(self.pipes):
            if pipe.poll(timeout=45.0): 
                ...
                if msg.get("status") != "READY": 
                    # FIX: Safely shutdown previously booted workers before raising
                    self.shutdown()
    ...
    def evolve(self, total_time_steps: int, dt: float, steps_per_dt: int, g_atom: float, g_mol: float, depth_per_step: int = 1, drop_rate: float = 0.0, hadron_hz: float = 0.2):
        ...
        print(f"Hadron Physics: {steps_per_dt} steps per dt ({dt}) -> J=1.0, hx=0.5, hz={hadron_hz}\n")
        ...
        for t in range(total_time_steps):
            ...
            if t == total_time_steps - 1:
                ...
                # Staggered benchmark requests to prevent massive RAM spikes
                bench_res = {}
