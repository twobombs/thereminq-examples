# -*- coding: us-ascii -*-
# 8-Qubit 2x2x2 Macroscopic Grid Annealing (Validation Scale)
...
# ==========================================
# 1. 8-QUBIT TOPOLOGY (2x2x2 LATTICE)
# ==========================================
def generate_8q_subvolume() -> Tuple[List[Tuple[int, int]], Dict[str, List[int]]]:
    lx, ly, lz = 2, 2, 2
    edges = []
...
# ==========================================
def persistent_island_worker_8q(
    device_id: int, 
    patch_idx: int, 
    num_qubits: int, 
    ...
    try:
        ...
        # FAST-PATH BIT EXTRACTOR
        def extract_specific_bits(samples: List[int], target_qubits: List[int]) -> np.ndarray:
            arr = np.array(samples, dtype=np.uint64)[:, None]
            mask = np.uint64(1) << np.array(target_qubits, dtype=np.uint64)
            bits = (arr & mask) > 0
            ...
            try:
                if action == "EVOLVE_AND_MEASURE_STATISTICAL":
                    ...
                    with gpu_semaphore:
                        ...
                        # 2. Z Clone - Fast Path Full Register Sampling
                        sim_z = QrackSimulator(clone_sid=sim.sid)
                        try:
                            z_samples = sim_z.measure_shots(all_q_list, corr_shots)
                        finally:
                            del sim_z
                        gc.collect() 
                        ...
                        try:
                            ...
                            x_samples = sim_x.measure_shots(all_q_list, corr_shots)
                        finally:
                            del sim_x
                        gc.collect() 
                        ...
                        try:
                            ...
                            y_samples = sim_y.measure_shots(all_q_list, corr_shots)
                        finally:
                            del sim_y
                        gc.collect() 

                    # Classical extraction outside the semaphore to free GPU instantly
                    Z_mat = extract_specific_bits(z_samples, all_boundary_qubits)
                    Z_mean = np.mean(Z_mat, axis=0)
                    Z_cov = np.cov(Z_mat, rowvar=False) + np.eye(num_bound) * 1e-6
                    
                    X_mat = extract_specific_bits(x_samples, all_boundary_qubits)
                    X_mean = np.mean(X_mat, axis=0)
                    X_cov = np.cov(X_mat, rowvar=False) + np.eye(num_bound) * 1e-6

                    Y_mat = extract_specific_bits(y_samples, all_boundary_qubits)
                    ...
                    with gpu_semaphore:
                        ...
                        # 2. Shot-based extraction of Z and ZZ correlators
                        sim_z_zz = QrackSimulator(clone_sid=sim.sid)
                        try:
                            zz_samples = sim_z_zz.measure_shots(all_q_list, 1024)
                        finally:
                            del sim_z_zz
                        gc.collect()
                        ...
                        try:
                            for q in range(num_qubits):
                                apply_h(sim_x, q)
                            x_samples = sim_x.measure_shots(all_q_list, 1024)
                        finally:
                            del sim_x
                        gc.collect()
...
# ==========================================
# 3. 8-QUBIT MACROSCOPIC ORCHESTRATOR
# ==========================================
class VolumetricHadronEngine8Q:
    def __init__(self, gpu_allocation: List[int], semaphore_limits: Dict[int, int], grid: Tuple[int, int, int] = (2, 2, 2), master_seed: int = 42):
        ...
        self.qubits_per_patch = 8
        self.intra_edges, self.boundaries = generate_8q_subvolume()
        ...
        self.csv_filename = "ground_state_energy_curve_8q.csv"
        self._init_csv()

        total_sites = self.num_patches * self.qubits_per_patch
        print(f"Initializing 8-Qubit Validation Engine...")
        ...
        try:
            for p_idx in range(self.num_patches):
                ...
                p = self.ctx.Process(
                    target=persistent_island_worker_8q,
                    args=(
    ...
    def sync_broadcast(self, action: str, kwargs_list: Optional[List[Dict]] = None, expected_status: Optional[str] = None) -> Dict[int, Any]:
        ...
        while pending:
            ...
            if not ready_pipes: 
                elapsed = 1800.0 - (deadline - time.monotonic())
    ...
    def shutdown(self) -> None:
        ...
        print("\nShutting down 8-Qubit Volumetric Engine...")
...
if __name__ == "__main__":
    ...
    engine = VolumetricHadronEngine8Q(
