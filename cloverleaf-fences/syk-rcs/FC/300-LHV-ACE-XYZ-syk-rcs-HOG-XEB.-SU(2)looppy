# -*- coding: us-ascii -*-
# from https://arxiv.org/html/2602.18080v1
...
def apply_cx(sim: Any, c: int, t: int) -> None:
    if hasattr(sim, 'cx'): 
        ...
        raise RuntimeError("No CX gate available in the current PyQrack version.")
...
def persistent_island_worker(
    ...
    # Ignore SIGINT in child processes to allow the orchestrator to handle shutdown
            ...
            try:
                if action == "EVOLVE_HADRONS":
                    ...
                    for _ in range(steps):
                        ...
                        for q in range(num_qubits): 
                            rotation_gates[0](sim, -2.0 * hx * dt, q)
                        # Longitudinal Field (Mass / String tension linear potential)
                        for q in range(num_qubits): 
                            rotation_gates[2](sim, -2.0 * hz * dt, q)
...
class HierarchicalHadronEngine:
    def __init__(self, device_ids: List[int], boundary_size: int = 4):
        ...
        for i, pipe in enumerate(self.pipes):
            if pipe.poll(timeout=45.0): 
                ...
                raise TimeoutError(f"Worker {i} initialization timed out.")
    ...
    def sync_broadcast(self, action: str, kwargs_list: Optional[List[Dict]] = None, timeout_secs: float = 300.0) -> Dict[int, Any]:
        ...
        while pending:
            ...
            if timeout <= 0: 
                raise TimeoutError("Workers timed out during synchronization.")
            ...
            for idx, pipe in pending:
                if pipe in ready_pipes:
                    try:
                        ...
                        raise RuntimeError(f"Worker {idx} connection unexpectedly crashed.")
                else: 
                    still_pending.append((idx, pipe))
            pending = still_pending
        return results

    def evolve(self, total_time_steps: int, dt: float, steps_per_dt: int, g_atom: float, g_mol: float) -> None:
            ...
            for atom_idx, patch_list in self.atoms.items():
                ...
                for pA in patch_list:
                    for bA in self.boundary_qubits:
                        ...
                        if fc_neighbors > 0:
                            # Normalize by the sqrt to scale coupling structurally
