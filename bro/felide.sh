...
# exported once so every child sees the same Qrack build; a mismatch between
# stages would change numbers with no error anywhere
[[ -n "${PYQRACK_SHARED_LIB_PATH:-}" ]] && export PYQRACK_SHARED_LIB_PATH
# lift the per-engine qubit cap so QUnit has no reason to elide; see
# xeb3d.py configure_qrack_env(). Not the fidelity guard -- that one would
# permit silent approximation.
export QRACK_MAX_CPU_QB="${QRACK_MAX_CPU_QB:--1}"
...
[[ -n "$QRACKLIB" ]] && export PYQRACK_SHARED_LIB_PATH="$QRACKLIB"
[[ -n "${PYQRACK_SHARED_LIB_PATH:-}" ]] && \
...
for QASM in "${CIRCUITS[@]}"; do
  ...
  if [[ ! -s "$MANIFEST" ]]; then
    ...
    if ! $VIZ patch "$QASM" --outdir "$STEM.patches" --cuts-from "$REFMAN" \
