# this is a runner that leverages pyqrackising to fold proteins in 3D
...
def solve_and_decode_protein(model_data, use_gpu):
    ...
    dimensions = model_data.get('dimensions', 3) # Default to 3D
    n = len(sequence)
    ...
    if dimensions == 3:
        ...
        for i in range(1, n):
            ...
            b2_idx = label_to_index.get(f'b_{i}_2', -1)
            b0 = binary_solution.get(b0_idx, 0)
            b1 = binary_solution.get(b1_idx, 0)
            b2 = binary_solution.get(b2_idx, 0)
            turns.append(turn_map.get((b2, b1, b0), 'F')) # Note the order b2,b1,b0
...
def process_and_log_result(run_id, energy, conformation, log_filename):
    ...
    with open(log_filename, 'a') as f:
        # Universal logger for 2D or 3D coordinates
...
def create_protein_folding_qubo_3d(sequence):
    ...
    Uses 3 bits for 5 moves: F, L, R, U, D.
    """
    n = len(sequence)
    qubo = defaultdict(float)
    
    # Helper to create variable names
    def b(i, turn_bit): return f'b_{i}_{turn_bit}'
    
    # Penalize invalid moves
    # The 3 bits (b2, b1, b0) allow 8 combinations, but we only use 5.
    # We penalize combinations (1,0,1), (1,1,0), (1,1,1)
    penalty = 10
    for i in range(1, n):
        # Penalty for b2=1 AND b1=1 (covers 110, 111)
        qubo[(b(i, 2), b(i, 1))] += penalty
        # Penalty for b2=1 AND b0=1 (covers 101, 111)
        qubo[(b(i, 2), b(i, 0))] += penalty

    # H-H contact energy bonus
    # This is a simplified model that rewards any 90-degree turn (L,R,U,D)
    # between two hydrophobic residues, as this brings them closer.
    # A turn occurs if any of the bits b0, b1, or b2 are 1.
    for i in range(1, n - 1):
        if sequence[i-1] == 'H' and sequence[i+1] == 'H':
            # A linear bias encourages bits to be 1, promoting a turn.
            qubo[(b(i, 0), b(i, 0))] -= 1.5 # Turn cost for b0=1 (L)
            qubo[(b(i, 1), b(i, 1))] -= 1.5 # Turn cost for b1=1 (R, U)
            qubo[(b(i, 2), b(i, 2))] -= 1.5 # Turn cost for b2=1 (D)
...
def create_protein_folding_qubo_2d(sequence):
    ...
    # Disallow backward move with a penalty
    for i in range(1, n):
        qubo[(b(i, 0), b(i, 1))] += 10

    # H-H interaction for 180-degree turn
    for i in range(1, n - 1):
        if sequence[i-1] == 'H' and sequence[i+1] == 'H':
            qubo[(b(i, 0), b(i, 0))] -= 1
            qubo[(b(i, 1), b(i, 1))] -= 1
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 4 and sys.argv[4].lower() == '2d': dimensions = 2
    ...
    if dimensions == 3:
        qubo = create_protein_folding_qubo_3d(hp_sequence)
    else:
        qubo = create_protein_folding_qubo_2d(hp_sequence)
