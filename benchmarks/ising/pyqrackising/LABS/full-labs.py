...
if __name__ == "__main__":
    ...
    # Extract the 32-bit solution for the original spins
    sequence_solution = best_bitstring[:n_nodes]

    print(f"Best cut value (incl. penalties): {best_cut_value}")
    print(f"Original sequence length: {n_nodes}")
    print(f"Sequence solution: {sequence_solution}")
    print(f"\nFull variable bitstring ({len(best_bitstring)} bits): {best_bitstring}")
    print(f"Full variable labels ({len(labels)} labels): {labels}")
