...
# ... (all previous fixes) ...
# FIX 16 (VISUAL): Set interpolated surface transparency to 0.9.
# FIX 17 (VISUAL): Render the interpolated mesh as a solid surface.
# FIX 18 (BUG): Corrected "Q_bits" typo in plot titles.
...
def main():
    ...
    # --- CUSTOM COLORMAP FIX (FIX 12) ---
    
    # Use a true log scale. Apply log after interpolation.
    log_grid_c = np.log(grid_c.ravel())
    
    # Replace -inf (from prob_zero=0) with a very small number for colormapping
    min_log_val = np.log(1e-9) # Our "zero" point for visual representation
    log_grid_c[np.isneginf(log_grid_c)] = min_log_val
    
    grid_mesh.pointdata["log_prob_zero"] = log_grid_c
    
    # 1. Apply the standard 'viridis' colormap
    grid_mesh.cmap("viridis", "log_prob_zero") 
    
    # Define the desired transparency for the interpolated surface
    # --- FIX 16: Changed alpha to 0.9 ---
    interpolated_surface_alpha = 0.9 # 0.0 = transparent, 1.0 = opaque
    ...
    try:
        ...
        # Find NaNs from interpolation (grid_c) OR from log(negative)
        ...
        if np.any(nan_indices):
            ...
            alphas = np.full(len(scalars), interpolated_surface_alpha) # Base transparency
            alphas[nan_indices] = 0.1 # Very transparent NaNs
            
            grid_mesh.alpha(alphas)
        else:
            # No NaNs, just set the global transparency
        ...
        grid_mesh.alpha(interpolated_surface_alpha) # Fallback
    # --- END FIX 7 ---
    
    # 2. Manually select points where prob_zero was 0 (now min_log_val)
    try:
        # Select points by the new "zero" value
        grid_mesh.select_points_by_scalar("log_prob_zero", min_log_val, 1e-9).color('black').alpha(1.0)
    except Exception as e:
    ...
    # <-- FIX 12: Update scalar bar title
    grid_mesh.add_scalarbar(title="Log(Probability of '0' State)", pos=((0.85, 0.1), (0.9, 0.9)), c='white')
    # --- END CUSTOM COLORMAP FIX ---

    # --- FIX 17: Ensure mesh renders as a surface ---
    grid_mesh.wireframe(False) # Explicitly turn off wireframe mode
    grid_mesh.lw(0)            # Set line width of mesh edges to 0
    # --- END FIX 17 ---

    plt = Plotter(
        # --- FIX 18: Corrected "Q_bits" typo ---
        title="OTOC Sweep: Time vs. Qubits and Depth (Gridded)",  
        bg='black',
        axes={
            'xtitle': 'Number of Qubits', # <-- Corrected typo
            'ytitle': 'Depth',
            # --- FIX 14: Update z-axis title and range ---
    ...
    # --- FIX 14: Use raw max_time ---
    z_offset = max_time * 0.005 
    raw_coords = df[['qubits', 'depth', 'time']].values # <-- Use raw time
    raw_coords[:, 2] += z_offset 

    # <-- FIX 13: Reduced radius from r=6 to r-3
    ...
    plt.add(grid_mesh) # Add the solid, semi-transparent grid
    ...
    try:
        ...
        # --- FIX 15: Add scale=3 for high-resolution screenshot ---
        plt.screenshot(OUTPUT_FILE, scale=3)  
