...
# RENDER FIX 2: Removed invalid camera arguments that were causing the blank screen.
# TIME SCALE FIX: Changed Z-axis from log(time) to linear time.
# AXIS RANGE FIX: Forced Z-axis (time) to start at 0.
# DOT SIZE FIX: Made the points smaller.
...
def parse_log_file(file_path):
    ...
    try:
        ...
        if total_seconds <= 0:
            ...
            return None
        # log_time = math.log(total_seconds) # <-- REMOVED
...
def main():
    ...
    if df.empty or df[['qubits', 'depth', 'time']].isnull().values.any(): # <-- CHANGED
    ...
    # --- Vedo Plotting Logic ---
    coords = df[['qubits', 'depth', 'time']].values
    scalars = df['prob_zero'].values
    pts = Points(coords, r=6) # <-- CHANGED: Smaller points
    ...
    # --- FIX: Removed the invalid camera=... argument ---
    plt.show(zoom=1.0)
    
    # 7. Save to PNG
    # This now runs *after* the window is closed, saving the last camera view.
    try:
